   ...
   public Player player_script; // for score
    ...
    void Start()
    {
     ...
     void SpawnBird()
     {
         ...
         float tempPos = UnityEngine.Random.Range(10f, 10f);
         Instantiate(Bird, new Vector3(tempPos, 3.16f, 0), Quaternion.identity);
    ...
    void Update()
    {
        ...
        if (transform.position.x < -9.2f)     // bird will destry when it will outoff screen
        {
            Destroy(gameObject);
     ...
     text.text =player_script.score.ToString();
       ...
       //text.text = (player_script.score + player2_script.score).ToString();
